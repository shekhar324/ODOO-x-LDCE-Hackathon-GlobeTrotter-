...
function PostForm({ initial = EMPTY_FORM, onSubmit, isPending, submitLabel, onClose, title }: PostFormProps) {
  const { user } = useAuth();
  const [form, setForm] = useState(initial);
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string>(initial.cover_image_url || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.story.trim()) {
      toast.error("Please provide both a title and a story.");
      return;
    }
    ...
    if (imageFile && user?.id) {
      setUploading(true);
      const { url, error } = await uploadCommunityImage(imageFile, user.id);
      setUploading(false);
      if (error) {
        toast.error("Image upload failed", { description: error + ". You can paste an image URL instead." });
      } else if (url) {
        cover_image_url = url;
      }
    }
  ...
  const field = (
    ...
    label: string,
    opts?: { rows?: number; placeholder?: string; labelColor?: string; bgColor?: string; borderColor?: string; focusColor?: string }
  ) => (
    <div>
      <label className={`text-xs block mb-1.5 font-mono ${opts?.labelColor || "text-neutral-300"}`}>{label}</label>
      {(opts?.rows ?? 1) > 1 ? (
        <textarea
          rows={opts?.rows}
          ...
          className={`w-full px-4 py-3 border rounded-xl text-white placeholder:text-neutral-500 focus:outline-none text-sm resize-none ${opts?.bgColor || "bg-white/5"} ${opts?.borderColor || "border-white/10"} ${opts?.focusColor ? `focus:border-${opts.focusColor}` : "focus:border-emerald-400"}`}
        />
      ) : (
        <input
          type="text"
          ...
          value={form[key]}
          onChange={(e) => setForm((f) => ({ ...f, [key]: e.target.value }))}
          className={`w-full px-4 py-3 border rounded-xl text-white placeholder:text-neutral-500 focus:outline-none text-sm ${opts?.bgColor || "bg-white/5"} ${opts?.borderColor || "border-white/10"} ${opts?.focusColor ? `focus:border-${opts.focusColor}` : "focus:border-emerald-400"}`}
        />
      )}
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div
        className="backdrop-blur-2xl bg-[#141414] border border-white/15 text-white max-w-xl w-full max-h-[90vh] overflow-y-auto rounded-3xl p-6 sm:p-8 space-y-6"
        data-lenis-prevent
      >
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-light text-white">{title}</h2>
          <button onClick={onClose} className="text-neutral-400 hover:text-white p-2 rounded-full hover:bg-white/10 transition-colors cursor-pointer">
            <IconX className="w-5 h-5" />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          {field("title", "Title *", { placeholder: "e.g. Hidden izakayas of Kyoto" })}
          {field("destination", "Destination", { placeholder: "e.g. Kyoto, Japan" })}
          {field("story", "Story & Atmosphere *", { rows: 4, placeholder: "Describe the experience, mood, culture, and memorable moments..." })}

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {field("dos", "Do's", { rows: 2, placeholder: "What travelers SHOULD do...", labelColor: "text-emerald-400", bgColor: "bg-emerald-500/5", borderColor: "border-emerald-500/20" })}
            {field("donts", "Don'ts", { rows: 2, placeholder: "What travelers SHOULD avoid...", labelColor: "text-rose-400", bgColor: "bg-rose-500/5", borderColor: "border-rose-500/20" })}
          </div>

          {field("tips", "Insider Tips", { placeholder: "e.g. Reserve train passes 3 weeks ahead" })}

          {/* Image upload section */}
          <div>
            <label className="text-xs text-neutral-300 block mb-1.5 font-mono">
              Cover Image <span className="text-neutral-500">(Optional)</span>
            </label>
            {imagePreview ? (
              <div className="relative w-full h-40 rounded-xl overflow-hidden mb-2">
                ...
                <button
                  type="button"
                  onClick={() => { setImageFile(null); setImagePreview(""); setForm((f) => ({ ...f, cover_image_url: "" })); }}
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-white/5 border border-white/20 rounded-xl text-sm text-neutral-300 hover:bg-white/10 hover:text-white transition-all cursor-pointer"
              >
            ...
            {!imageFile && (
              <div className="mt-2">
                <input
                  ...
                  onChange={(e) => { setForm((f) => ({ ...f, cover_image_url: e.target.value })); setImagePreview(e.target.value); }}
                  className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-neutral-500 focus:outline-none focus:border-emerald-400 text-sm"
                />
              </div>
            )}
          </div>
          ...
          >
            {uploading ? "Uploading image..." : isPending ? "Publishing..." : submitLabel}
          </button>
        </form>
      </div>
    </div>
...
export default function CommunityPage() {
  ...
  const anyModalOpen = isCreateModalOpen || !!editingPost || !!selectedPost;
  ...
  const handleCreatePost = async (formData: typeof EMPTY_FORM) => {
    try {
      await createPostMutation.mutateAsync({
        title: formData.title.trim(),
        destination: formData.destination.trim() || undefined,
        story: formData.story.trim(),
        how_it_went: formData.how_it_went.trim() || undefined,
        recommendations: formData.recommendations.trim() || undefined,
        dos: formData.dos.trim() || undefined,
        donts: formData.donts.trim() || undefined,
      ...
      toast.error("Failed to publish story", { description: message });
    }
  };

  const handleUpdatePost = async (formData: typeof EMPTY_FORM) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "An unexpected error occurred.";
      toast.error("Failed to update story", { description: message });
    }
  };
  ...
  const handleDeletePost = async (postId: string) => {
    try {
      ...
      const message = err instanceof Error ? err.message : "An unexpected error occurred.";
      toast.error("Failed to delete story", { description: message });
    }
  };
    ...
    <AuthGuard>
      <div className="min-h-screen bg-[#0e0e0e] text-[#e2e2e2] flex flex-col font-sans selection:bg-[#c3eeb4] selection:text-[#002200]">
        ...
        <main className="flex-1 max-w-[1200px] mx-auto w-full pt-32 sm:pt-40 pb-32 px-6 md:px-12 z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-apple-pill text-xs text-emerald-300 mb-4">
                ...
                <span className="font-mono tracking-wide uppercase text-[11px]">Global Collective & Journal</span>
              </div>
          ...
          {/* Stories Grid */}
          {isLoading ? (
            <div className="py-20 text-center text-[#becabb] font-mono animate-pulse">Loading collective stories...</div>
          ) : isError ? (
            <div className="py-20 text-center text-red-400 font-mono">Failed to load community posts. Please refresh or try again later.</div>
          ) : posts.length === 0 ? (
            <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-12 text-center max-w-md mx-auto">
              ...
              <p className="text-xs text-[#becabb] mb-6">Be the first traveler to share your journey, insights, and local recommendations.</p>
              <button
                onClick={() => setIsCreateModalOpen(true)}
                  ...
                  >
                    <div>
                      ...
                      <div className="p-6">
                        <div className="flex justify-between items-center mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-neutral-500">{new Date(post.created_at).toLocaleDateString()}</p>
                            </div>
                          </div>
                          {isAuthor && (
                        ...
                        <h3 className="text-xl font-light text-white mb-2 line-clamp-2">{post.title}</h3>
                        <p className="text-xs text-[#becabb] leading-relaxed line-clamp-4 mb-4">{post.story || post.description}</p>
        ...
        {selectedPost && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <div
              className="backdrop-blur-2xl bg-[#141414] border border-white/15 text-white max-w-2xl w-full max-h-[85vh] overflow-y-auto rounded-3xl p-6 sm:p-8 space-y-6"
              data-lenis-prevent
            >
              <div className="flex justify-between items-start">
                <div>
                  {selectedPost.destination && (
                    <span className="text-xs font-mono text-emerald-400 uppercase tracking-widest block mb-1">{selectedPost.destination}</span>
                  )}
                  <h2 className="text-2xl sm:text-3xl font-light">{selectedPost.title}</h2>
                </div>
                <button onClick={() => setSelectedPost(null)} className="text-neutral-400 hover:text-white p-2 rounded-full hover:bg-white/10 transition-colors cursor-pointer">
                  <IconX className="w-5 h-5" />
                </button>
              </div>
              {selectedPost.cover_image_url && (
                <div className="h-64 w-full rounded-xl overflow-hidden">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={selectedPost.cover_image_url} alt={selectedPost.title} className="w-full h-full object-cover" />
                </div>
              )}
              <div className="space-y-4 text-sm text-[#becabb] leading-relaxed">
                <div>
                  <h4 className="text-white font-medium text-xs uppercase tracking-wider mb-2 font-mono">The Experience</h4>
                  <p className="whitespace-pre-line">{selectedPost.story || selectedPost.description}</p>
                </div>
                {selectedPost.how_it_went && (
                  <div>
                    <h4 className="text-white font-medium text-xs uppercase tracking-wider mb-2 font-mono">How It Went</h4>
                    <p className="whitespace-pre-line">{selectedPost.how_it_went}</p>
                  </div>
                )}
                {selectedPost.recommendations && (
                  <div>
                    <h4 className="text-white font-medium text-xs uppercase tracking-wider mb-2 font-mono">Top Recommendations</h4>
        ...
        {isCreateModalOpen && (
          <PostForm
            ...
            submitLabel="Publish to Community"
        ...
        {editingPost && (
          <PostForm
            ...
            initial={{
              ...
              destination: editingPost.destination || "",
              story: editingPost.story || "",
              how_it_went: editingPost.how_it_went || "",
              recommendations: editingPost.recommendations || "",
              dos: editingPost.dos || "",
              donts: editingPost.donts || "",
              tips: editingPost.tips || "",
